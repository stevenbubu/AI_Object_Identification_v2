...
class SelfTracker(object):
    ...
    def tracking_by_hands(self, full_img, hand_detections=None):
        if self.loss_track or hand_detections is None:
            ...
            self.cur_center = np.mean(hand_detections, axis=0, dtype=np.int)
            crop_h = np.max(hand_detections[:, 0]) - np.min(hand_detections[:, 0])
            crop_w = np.max(hand_detections[:, 1]) - np.min(hand_detections[:, 1])
    ...
    def _crop_image(self, full_img, center, size):
        h_offset = size[0] % 2
        w_offset = size[1] % 2
        self.bbox = [int(max(0, center[0]-size[0]//2)), int(min(self.img_shape[0], center[0]+size[0]//2+h_offset)),
                int(max(0, center[1]-size[1]//2)), int(min(self.img_shape[1], center[1]+size[1]//2+w_offset))]
