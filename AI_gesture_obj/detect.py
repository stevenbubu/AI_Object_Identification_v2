...
def main(argv):
    ...
    while True:
        ...
        if (FLAGS.display):
            ...
            #             cv2.cvtColor(crop_img[0], cv2.COLOR_RGB2BGR))
            # cv2.imshow("Left Hand", 
            #             cv2.cvtColor(crop_img[1], cv2.COLOR_RGB2BGR))
            cv2.imshow("Right Hand gesture", 
                        cv2.cvtColor(gesture_img[0], cv2.COLOR_RGB2BGR))
            cv2.imshow("Left Hand gesture", 
                        cv2.cvtColor(gesture_img[1], cv2.COLOR_RGB2BGR))
            # cv2.imwrite('test.jpg',cv2.cvtColor(image_np, cv2.COLOR_RGB2BGR))
            # cv2.waitKey(1000)  
            # cv2.imshow('crop_img_tmp',
            #         cv2.cvtColor(crop_img_tmp, cv2.COLOR_RGB2BGR))
            # cv2.imshow('crop_img_tmp',
            #         cv2.cvtColor(crop_img_tmp, cv2.COLOR_RGB2BGR))
            # cv2.imshow('MR_hand_2',
            #         cv2.cvtColor(crop_img[1], cv2.COLOR_RGB2BGR))    
            # cv2.imshow('YL_hand_3',
            #         cv2.cvtColor(crop_img[2], cv2.COLOR_RGB2BGR)) 
            # cv2.imshow('YR_hand_4',
            #         cv2.cvtColor(crop_img[3], cv2.COLOR_RGB2BGR))   
