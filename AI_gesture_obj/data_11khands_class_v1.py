...
def flipHImg(path, folders=[], flips=[]):
    flipfd = []
    ...
    delOldFolders(path, folders=folders, name=flipfd)


def rotateImg(path, folders=[], rotates=[]):
    rotatefd = []
    ...
    delOldFolders(path, folders=usefd, name=rotatefd)
    
   
def scaleImg(path, folders=[], scales=[]):
    scalefd = []
    ...
    delOldFolders(path, folders=usefd, name=scalefd)
    recordHandLoc(path, name=scalefd)


def luminImg(path, folders=[], lumins=[]):
    luminfd = []
    ...
    delOldFolders(path, folders=usefd, name=luminfd)


def noiseImg(path, folders=[], noises=[]):
    noisefd = []
    ...
    delOldFolders(path, folders=usefd, name=noisefd)
...
def hand_dataset_split(path):
    ...
    # imgnum = min(len(DorsalRight), len(PalmarRight), len(DorsalLeft), len(PalmarLeft))
    ...
    flipHImg(path=imgfdpath, folders=folders, flips=[0, 1])
    ...
    rotateImg(path=imgfdpath, folders=folders, rotates=[0, 45, 90, 135, 180, 225, 270, 315])
    ...
    scaleImg(path=imgfdpath, folders=folders, scales=[0.25, 0.50, 0.75, 1.00, 1.25])
    ...
    luminImg(path=imgfdpath, folders=folders, lumins=[0.1, 1.0, 5.0])
    ...
    noiseImg(path=imgfdpath, folders=folders, noises=['normal', 'gaussian', 's&p'])
...
save_txt(FLAGS.filepath+'/time.txt', total_spend, string="\ntotal: ")
